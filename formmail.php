<?php
$FM_VERS = "8.29";      // script version

/* ex:set ts=4 sw=4 et:
 * FormMail PHP script from Tectite.com.  This script requires PHP 4 or later.
 * Copyright (c) 2001-2011 Root Software and Open Concepts (Vic) Pty Ltd
...
$TARGET_EMAIL = array("^gtjane\@kinect\.co\.nz$");

            /* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","graemetjane@gmail.com");
...
$CSVDIR = "/home/nativeor/public_html/csvdir";   // directory for csv files; empty string to
                                    // disallow csv files
...
$TEMPLATEDIR = "/home/nativeor/public_html";                  // directory for template files; empty string
...
$ATTACK_DETECTION_DUPS = array("realname","number","street","country",
                                "postcode","city","email");
...
$ATTACK_DETECTION_REVERSE_CAPTCHA = array("dg7er3"=>"pew6hy","k1er7u"=>"","zip"=>"");
...
function LoadBuiltinLanguage()
{
    ...
    $aMessages[MSG_NO_DATA_PAGE] =  'This URL is a Form submission program.\n'.
        ...
        // Parameters: none
    $aMessages[MSG_REQD_ERROR] = 'The Order Form required the following '.
                                 'values that you did not enter:\n';
...
class   FieldManager
{
     ...
     */
    function GetSafeFieldValue($s_fld,$s_array_sep = ";")
    {
            //
            ...
            // By specifying the character set, we trgger the use of htmlspecialchars
    ...
    //
function    LineFolding($s_str,$i_max_line = RFCLINELEN,$s_before,$s_after,$s_fold)
{
...
function    QPEncode($s_str,$i_max_line)
{
        ...
        //
    $s_str = str_replace(array('%20','%'),array(' ','='),rawurlencode($s_str));
    if ($i_max_line < 0)
        return ($s_str);
    else
    {
...
class   NetIO
{
    ...
    function    Open()
    {
        ...
        if (!$this->Resolve())
            return (FALSE);
        FMDebug("Starting fsockopen");
        $f_sock = FALSE;
            //
            // run through the list of IPs until we find one that connects
            //
        foreach ($this->_aIPs as $s_ip)
        {
            FMDebug("Trying IP $s_ip, timeout ".$this->GetConnectionTimeout());
            if (($f_sock = @fsockopen($this->GetPrefix().$s_ip,$this->GetPort(),
                                $errno,$errstr,$this->GetConnectionTimeout())) !== FALSE)
                break;
        }
        if ($f_sock === FALSE)
        {
            FMDebug("fsockopen failed: $errno $errstr");
            return ($this->_SetError($this->nErrSocket,$errno,$errstr));
        }
        $this->_fSock = $f_sock;
        FMDebug("Done fsockopen");
        return (TRUE);
    }
...
function SubstituteValueWorker($a_matches,$s_repl,$b_html = true)
{
        ...
        !$SubstituteFields->TestFieldEmpty($s_name,$s_mesg))
    {
        if ($b_html)
                    ...
                    //
            $s_value = $SubstituteFields->GetSafeFieldValue($s_name,$s_list_sep);
        else
...
class   AutoResponder
{
    ...
    function    AutoResponder()
    {
        ...
        $this->_iCaptchaType = $this->_iNone;
            ...
            //  4.  If HTMLTemplate and PlainTemplate are *not* set and PlainFile
            //      is set:
            ...
            //  or by ensuring Reverse CAPTCHA is performed (for PlainFile).
            //
